function receiveData(item) {
    if (item.ruleFile) {
        document.getElementById("oldRuleFile").value = item.ruleFile;
    }
    if (item.fileType) {
        // fileType could be used if needed
    }
    if (item.NewRuleFileName) {
        document.getElementById("ruleFileName").value = item.NewRuleFileName;
    }
...
function getUrlParams() {
    ...
    return {
        ruleFile: params.get('ruleFile') || '',
        newRuleFileName: params.get('newRuleFileName') || params.get('NewRuleFileName') || '',
        oldRuleFileName: params.get('oldRuleFileName') || params.get('OldRuleFileName') || ''
    };
}
...
function initFromUrlParams() {
    ...
    if (params.ruleFile) {
        document.getElementById("oldRuleFile").value = params.ruleFile;
    }
    if (params.newRuleFileName) {
        document.getElementById("ruleFileName").value = params.newRuleFileName;
    }
    if (params.oldRuleFileName) {
        document.getElementById("oldRuleFile").value = params.oldRuleFileName;
    }
}
...
function toggleOldRuleFileDetail() {
  const updateAction = document.getElementById("updateAction").value;
  const oldRuleFileDetail = document.getElementById("oldRuleFileDetail");
  const newEvtTypeFileDiv = document.getElementById("newEvtTypeFileDiv");

  if (updateAction === "New") {
      oldRuleFileDetail.style.display = "none";
      newEvtTypeFileDiv.style.display = "block"
  } else {
      oldRuleFileDetail.style.display = "block";
      newEvtTypeFileDiv.style.display = "none"
  }
}
...
function startUpdate() {
  ...
  if (updateActionElem.value !== "New") {
    const ruleFileName = document.getElementById("oldRuleFile").value.trim();
    if (ruleFileName === "") {
       logElem.textContent = "Please select a rule file to update";
       return;
    }
    ...
    const evtTypeFileNameElem = document.getElementById("newEvtTypeFileName");
    formData.append("update_action", updateActionElem.value);
    formData.append("new_rule_filename", ruleFileNameElem.value);
    formData.append("new_eventtype_filename", evtTypeFileNameElem.value);
    formData.append("error_rule_status_filename", errorRuleStatusFileName.value);
    URL =  url_header + "/update";
  }
  ...
  .then(data => {
    if(data.code === 0) {
       ...
       if (updateActionElem.value === "New") {
          document.getElementById("newEvtTypeFileName").value = data.NewEvtTypeFileName;
          updateRuleFileList(data.NewEvtTypeFileName, "add")
       }
     } else {
...
async function uploadRuleXml() {
  ...
  if (response.success) {
	...
	alert("Upload Selected Rule File successfully")
  } else {
...
function downloadEvtTypeRule() {
 let url_header = getUrlHeader();
 const ruleFileName = document.getElementById("newEvtTypeFileName").value;		
 URL = url_header + "/download/" + ruleFileName;
 window.open(URL);
}
...
function showErrRules() {
  ...
  const currentItem = {"ruleFile": errFileName, "fileType": "error", "CommitId": commitId}
  win.onload = function () {
...
function checkRules() {
  ...
  const currentItem = {"ruleFile": newRuleFileName, "fileType": "rule", "CommitId": commitId}
  win.onload = function () {
      if (typeof win.receiveData === "function") {
         win.receiveData(currentItem);
      } else {
